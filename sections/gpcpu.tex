...
Für die Inputs und Outputs aller Layer wurde die Klasse \texttt{Tensor} erstellt. Diese beinhaltet ein Array mit Floats sowie drei Integer mit den Namen \texttt{x}, \texttt{y} und \texttt{z}. Die Größe des Arrays wird anhand der Wertes der Integer bestimmt. Beim Aufrufen des Constructors wird diese Größe übergeben. Die Klasse bietet Funktionen, um die Adresse des Arrays an bestimmten Stellen zu finden. Desweiteren gibt es Funktionen, um die Größe des Tensors in die drei Dimensionen zu erhalten.

Im Programm gibt es ein union \texttt{Layer}. Dieses beinhaltet drei verschiedene Layertypen, den Convolutional Layer, den Max Pooling Layer und den Fully Connected Layer. Jede dieser Layertypen ist eine extra Klasse. 
Alle dieser Klassen beinhaltet die Methoden \texttt{forward()}, \texttt{backward()} und \texttt{fix\_weights()}. Die Funktionen \texttt{forward()} und \texttt{backward()} nehmen keine Übergabeparameter an. Erstere wird aufgerufen, um die Inputs der Neuronen durch das Netzwerk zu propagieren. Letztere wird aufgerufen, um die Fehler zurück zu propagieren. Die Deltas der Gewichte und der Biases werden in extra Tensoren gespeichert. Durch die Funktion \texttt{fix\_weights()} werden die Deltas benutzt, um die Gewichte und die Biases anzupassen. Dabei wird die Batch-Größe angegeben sowie die Trainingsrate. Die Deltas müssen für die Backpropagation mit einer Batch zwischengespeichert werden, weshalb die extra Tensoren für die Deltas existieren.
Tensoren werden von den Klassen als Activation eines Neurons, als Gewichtsliste oder als Biasliste verwendet. Da die Klassen lediglich einen Pointer auf das Tensor-Objekt besitzen, können mehrere Layer-Klassen auf den selben Tensor zugreifen. Der Pointer zum Output innerhalb des vorherigen Layers hat die selbe Adresse wie der Pointer zur Activation des dahinter liegenden Layers.
